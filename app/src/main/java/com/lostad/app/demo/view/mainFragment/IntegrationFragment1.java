...
public class IntegrationFragment1 extends BaseFragment {
	...
	private void initTabs(LayoutInflater inflater) {
			...
			ContactFragment contacts = new ContactFragment();

			LCIMConversationListFragment conversation=new LCIMConversationListFragment();

		   fragments.add(conversation);
		   fragments.add(contacts);
		int width = screenWidth / fragments.size();
		View v1 = createTabItem(inflater,width, "会话");
		View v2 = createTabItem(inflater,width, "联系人");
	...
	private class MyPageAdapter extends FragmentPagerAdapter {
		...
		public Fragment getItem(int index) {
			Fragment f = (Fragment) fragments.get(index);
			return f;
		}
